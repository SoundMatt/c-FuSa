...
/*
 * CFUSA_COMMANDS is the single source of truth main() dispatches subcommands
 * through, and cmd_help() enumerates to print top-level usage. Both live
 * here (rather than in main.c, which only contains main() and therefore
 * cannot be linked into the unit-test binaries) so the dispatch table and
 * help text stay unit-testable.
 */
